...
a lot of code to do not much, which makes it easy to introduce mistakes in our code. Another mistake in this example
would be to start the loop over ranks from 0, which would cause a deadlock! It's actually quite a common mistake for new
MPI users to write something like above.
...
There are lots of use cases for broadcasting data. One common case is when data is sent back to a "root" rank to
